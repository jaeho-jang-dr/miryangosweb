/**
 * GET /api/clinical/hira/drug-usage — 의약품 처방정보 조회 (기본)
...
import { getDrugPrescriptionInfo } from '@shared/lib/hira/drug-usage';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getDrugPrescriptionInfo({
    ...
    await writeAuditLog({
      action: 'read',
      collection: 'hira_drug_usage',
      documentId: searchParams.get('mdcinCpntNm') || searchParams.get('mdcinPrductNm') || 'search',
      userId: decoded.uid,
      userEmail: decoded.email || undefined,
      description: `의약품 처방정보 조회: ${searchParams.get('mdcinCpntNm') || searchParams.get('mdcinPrductNm') || 'all'}`,
    });

    return NextResponse.json(result);
  } catch (error: unknown) {
    if (error instanceof HiraError) {
      console.error(`[HIRA/drug-usage] ${error.code}: ${error.message}`);
    ...
    console.error('[HIRA/drug-usage] Error:', message);
