/**
 * GET /api/clinical/hira/drug-usage/prescription — 의약품 처방 통계
...
export async function GET(req: NextRequest) {
  try {
    ...
    await writeAuditLog({
      action: 'read',
      collection: 'hira_drug_prescription',
      documentId: searchParams.get('mdcinCpntNm') || searchParams.get('mdcinPrductNm') || 'search',
      userId: decoded.uid,
      userEmail: decoded.email || undefined,
      description: `의약품 처방통계 조회: ${searchParams.get('mdcinCpntNm') || 'all'}`,
    });

    return NextResponse.json(result);
  } catch (error: unknown) {
    if (error instanceof HiraError) {
      console.error(`[HIRA/drug-prescription] ${error.code}: ${error.message}`);
    ...
    console.error('[HIRA/drug-prescription] Error:', message);
