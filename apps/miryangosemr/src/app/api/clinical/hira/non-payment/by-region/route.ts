/**
 * GET /api/clinical/hira/non-payment/by-region — 비급여 지역별 가격 조회
 * Query: npayKorCd (필수), sidoCd, sgguCd, pageNo, numOfRows
 */
...
import { getNonPaymentCostsByRegion } from '@shared/lib/hira/non-payment';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getNonPaymentCostsByRegion({
      npayKorCd,
    ...
    await writeAuditLog({
      action: 'read',
      collection: 'hira_non_payment_region',
      documentId: npayKorCd,
      userId: decoded.uid,
      userEmail: decoded.email || undefined,
      description: `비급여 지역별 가격 조회: ${npayKorCd}`,
    });

    return NextResponse.json(result);
  } catch (error: unknown) {
    if (error instanceof HiraError) {
      console.error(`[HIRA/non-payment/by-region] ${error.code}: ${error.message}`);
    ...
    console.error('[HIRA/non-payment/by-region] Error:', message);
