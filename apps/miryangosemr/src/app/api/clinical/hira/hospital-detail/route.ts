/**
 * GET /api/clinical/hira/hospital-detail — 의료기관 개설/폐업/인력 현황
...
import { getHospitalListStatus } from '@shared/lib/hira/hospital-detail';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getHospitalListStatus({
    ...
    await writeAuditLog({
      action: 'read',
      collection: 'hira_hospital_detail',
      documentId: searchParams.get('ykiho') || searchParams.get('yadmNm') || 'search',
      userId: decoded.uid,
      userEmail: decoded.email || undefined,
      description: `의료기관 상세(인력) 조회: ${searchParams.get('yadmNm') || searchParams.get('sidoCd') || 'all'}`,
    });

    return NextResponse.json(result);
  } catch (error: unknown) {
    if (error instanceof HiraError) {
      console.error(`[HIRA/hospital-detail] ${error.code}: ${error.message}`);
    ...
    console.error('[HIRA/hospital-detail] Error:', message);
