...
export function LiveTranscript({ segments, isRecording, isAnalyzing }: LiveTranscriptProps) {
    ...
    const handleCopyTranscript = useCallback(() => {
        const text = segments
            .filter(s => s.isFinal)
            .map(s => {
                const label = s.speaker === 'doctor' ? '[의사]' : s.speaker === 'patient' ? '[환자]' : '';
                return `${label} ${s.text}`;
            })
            .join('\n');
                ...
                <div className="flex gap-2">
                    {segments.length > 0 && (
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1 bg-slate-50 text-slate-500 hover:text-slate-700 rounded-full text-[10px] font-bold uppercase tracking-wider border border-slate-100 transition-colors"
                        >
                            {copied ? 'Copied!' : 'Copy'}
                        </button>
                    )}
