...
import { AudioRecorder, type RecordingState } from '@shared/lib/voice/audio-recorder';
...
export function AudioRecorderComponent({
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const supported = !!(navigator.mediaDevices?.getUserMedia) &&
                ...
                !!((window as unknown as Record<string, unknown>).webkitSpeechRecognition || (window as unknown as Record<string, unknown>).SpeechRecognition);
    ...
    const handleStart = async () => {
        try {
            ...
            try {
                ...
                // Mic visualization is non-critical
            }
    ...
    const handleStop = async () => {
        try {
            ...
            const message = err instanceof Error ? err.message : '녹음 정지에 실패했습니다.';
            setError(message);
        }
    };
    ...
    return (
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
            ...
            {!isSupported && (
                <div className="mx-6 mt-4 p-3 bg-amber-50 border border-amber-100 rounded-2xl flex items-center gap-3">
            ...
            {recordingState.isRecording && (
                <div className="mx-6 mt-4">
                    <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Mic Level</span>
                        <span className="text-[10px] text-slate-400 font-bold">{Math.round(micLevel)}%</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all duration-75 ${
                                micLevel > 70 ? 'bg-rose-500' : micLevel > 30 ? 'bg-emerald-500' : 'bg-slate-300'
                            }`}
