---
description: 코드 품질을 개선하기 위해 코드 최적화 특수 기동대(TF)를 실행합니다.
---

# 코드 최적화 TF (Code Optimization Task Force)
...
## 👔 프로젝트 매니저 프로세스 (Project Manager Process)

사용자가 코드를 제공하면 다음 단계로 진행하십시오:

1.  **[1단계: 정리]** `Code Janitor` 페르소나를 사용하여 불필요한 코드와 주석을 정리하십시오.
2.  **[2단계: 구조화]** `Architect` 페르소나를 사용하여 구조적 개선과 모듈화를 수행하십시오.
3.  **[3단계: 최적화]** `Performance Engineer` 페르소나를 사용하여 성능 병목을 해결하십시오.
4.  **[4단계: 보안]** `Security & QA Officer` 페르소나를 사용하여 예외 처리와 보안을 강화하십시오.

**최종 결과:**
최종적으로 모든 단계가 반영된 '최적화 완료 코드(Final Optimized Code)'를 출력하고, 각 단계별 변경 사항을 요약하십시오.
