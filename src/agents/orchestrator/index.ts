...
export class AgentOrchestrator {
  ...
  async executeSquad(
    ...
    const workflow = (customWorkflow || squadConfig.defaultWorkflow || squadConfig.agents) as AgentRole[];
...
// Commands wrapper for easier usage
export const commands = {
  listAgents: (squad?: SquadType) => squad ? getAgentsBySquad(squad) : allAgents,
  listSquads: () => squadConfigs,
  showAgent: (role: AgentRole) => getAgentByRole(role),
  runAgent: (role: AgentRole, input: string) => orchestrator.executeAgent(role, input),
  runSquad: (squad: SquadType, input: string) => orchestrator.executeSquad(squad, input),
  runWorkflow: (name: string, input: string) => {
    const workflow = (workflows as any)[name];
    if (workflow) return orchestrator.executeWorkflow(workflow(input));
    throw new Error(`Workflow ${name} not found`);
  },
  showHistory: (limit?: number) => orchestrator.getTaskHistory(limit),
  showStats: () => orchestrator.getStatistics(),
  welcome: () => "Welcome to Claude Agent System",
  help: () => "Available commands: listAgents, runAgent, runSquad, ..."
};
